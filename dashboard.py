...
# Valuation math + numeric helpers live in valuation.py (pure, unit-tested — see
...
def get_quant_score(ticker):
    ...
    # a low-leverage tilt (robustly positive, t=2.5) and a small size tilt. Quality/cash-flow trimmed
    # to make room. (A rate-aware growth DAMPENER was tried and REVERTED: a 2011-2024 backtest showed
    # it couldn't fix the 2021-22 growth inversion — that was driven by rates RISING mid-hold, not the
    # rate level at purchase — and it mildly hurt moderate-rate growth years. Evidence didn't support it.)
    investment_score = (
        quality_score * 0.26 +
        growth_score * 0.22 +
...
@st.cache_data(ttl=12 * 3600, show_spinner=False)
def _fetch_market_valuation_cached():
...
def fetch_market_valuation():
    """Cache-poisoning guard: st.cache_data would happily hold a FAILED fetch (all-None dict)
    for the full 12h TTL — one transient network hiccup and the whole forecast page reads
    'unavailable' all day (observed live 2026-07-24). If the fetch came back empty, evict it
    so the next rerun retries instead of re-serving the failure."""
    out = _fetch_market_valuation_cached()
    if not out or out.get("cape") is None:
        try:
            _fetch_market_valuation_cached.clear()
        except Exception:
            pass
    return out
...
def aiae_forecast():
    """Cache-poisoning guard (same pattern as fetch_market_valuation): a None result means a
    FRED/Z.1 fetch failed — evict it (and compute_aiae's cache) so the next rerun retries
    instead of serving the failure for 24h."""
    out = _aiae_forecast_cached()
    if out is None:
        try:
            _aiae_forecast_cached.clear()
            compute_aiae.clear()
        except Exception:
            pass
    return out
...
def _compute_forecast_bundle():
    ...
    The bundle self-reports `degraded=True` when key sources failed (recession signals missing,
    or the consensus fell back to a single method). A degraded bundle is served rather than
    nothing, but it is never allowed to poison the committed cache or stick around: the loader
    skips degraded caches and this live compute retries every 30 minutes instead of 12-24h."""
    ...
    cons_n = (bundle["cons"] or {}).get("n", 0)
    bundle["degraded"] = bundle["rec"] is None or bundle["ai"] is None or cons_n < 2
    return bundle
    ...
    try:
        ...
        if gen:
            ...
            if age > FORECAST_CACHE_MAX_AGE_DAYS:
                return None
        if b.get("degraded"):
            return None   # a cache built during a source outage — recompute live instead
...
def render_market_forecast():
    ...
    if data.get("degraded"):
        st.warning("⚠️ Some data sources (FRED / Fed Z.1) are unreachable right now, so parts of this forecast are missing or single-method. Nothing below is wrong — it's just incomplete. Retries automatically within ~30 minutes.")
...
def verdict_context(mos, sector=None, ticker_news=None):
    ...
    if read and cons:
        mv = read["verdict"]                    # Cheap … Extreme
        expensive = mv in ("Elevated", "Very expensive", "Extreme")
        cheap_mkt = mv in ("Cheap", "Reasonable")
        cr = cons.get("consensus")
        base = f"The broad market is **{mv.lower()}** (≈{cr:+.0f}%/yr expected over the next decade)."
        if mos == "cheap" and expensive:
            base += " So even though this looks cheap, the market tide is against it — it has to win on its own merits. Favor dollar-cost averaging over a big lump-sum entry."
        elif mos == "cheap" and cheap_mkt:
            base += " A cheap stock in a reasonably-priced market — a rare tailwind at both levels."
        elif mos == "expensive" and expensive:
            base += " An expensive stock in an expensive market — thin margin of safety at either level; the highest-caution setup."
        elif mos == "expensive":
            base += " The premium here is company-specific — the market overall isn't stretched."
        else:
            base += " A roughly fair backdrop for a fairly-priced name."
        layers.append(("🌐 Market backdrop", base))
    ...
    if ticker_news:
        ...
        if tl and tl.get("rollup"):
            layers.append(("🗞️ News flow", f"Recent catalysts: **{tl['rollup']['flow']}**. {tl['rollup']['read']}"))
    return layers
...
def render_investment_verdict(row, ticker_news=None):
    ...
    news). Everything in the tabs below is the evidence behind it."""
    ...
    with st.container(border=True):
        ...
        st.caption("Bottom-up company read, placed inside the top-down market backdrop and near-term flow — shown as layers (different time horizons), not blended into one false-precise number. Research aid, not advice.")
...
def build_stock_pdf(row, verdict, bull, bear):
    ...
    # fpdf2 gotcha: multi_cell's default new_x leaves the cursor at the cell's RIGHT edge, so a
    # second consecutive multi_cell(0, ...) has zero width and raises "Not enough horizontal
    # space". Explicit LMARGIN/NEXT makes each bullet start a fresh full-width line — this is
    # why the PDF export failed for any stock with 2+ bull or bear points.
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 7, "Bull case", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 10)
    for b in bull:
        pdf.multi_cell(0, 5, _latin1(f"- {b}"), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(1)
    pdf.set_font("Helvetica", "B", 12)
